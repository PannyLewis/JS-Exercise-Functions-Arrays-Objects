...
function temperatureInF(temp, Unit) {
  if (Unit === "F") {
    ...
    let tempF = temperatureCtoF(temp);
    return `${tempF}F`;
  }
}
...
function getLastCarInfo(inventory) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    // if the index is same as the id of the car, then call it cars that the will be your new array
...
//  this variable has to be somewhat global, it must be set or declared first before the looping happens.  You getting the bucket ready before you fill it with water from the well.  You are saying I just created an empty array.  Go through the inventory array and fetch some items.  Then push them into this new empty array.

let carYear = [];

function getModelYears(inventory) {
  // let carYear = []; will work outside of the function block or inside the function block but won't work inside the loop.

  for (let i = 0; i < inventory.length; i++) {
    // you can't just declare one variable and make it equal to the an inventory
    // let carYear = inventory[i].car_year; won't work
...
function getCarInfoById(array, id) {
  for (let i = 0; i < array.length; i++) {
    // if the 2nd parameter/argument that the user put in is equal to the id of the car
    if (id === array[i].id) {
      // loop through the array and if you find that the id is the same as the index then place it into a new array named carId.
      let carId = array[i];
...
function getOlderCars(array, maxYear) {
  // get the empty array ready
  let olderCars = [];
  // make a loop, tell it go through the array, and what to look for
  for (let i = 0; i < array.length; i++) {
    // you only want the cars that the year is older than whatever the argument maxYear is
    if (maxYear >= array[i].car_year) {
      // you have to tell it what to do with the info
      // put the info collected into the new empty array

      olderCars.push(
        array[i]
      ); /*it won't push the entire array, only the items >= maxYear */
    }
  }
  return olderCars;
}
...
function getGermanCars(array) {
  // first creat an empty to throw in the new data
  let germanCars = [];
  // loop through the inventory array and tell it what to look for
  for (let i = 0; i < array.length; i++) {
    // that car models must either or so we use the symbol ||
    if (
      ...
      /*if it matches then push it into the new array*/
    }
  }
  // after you loop thorugh and found matching item to save into the new array then you return the array here.
  return germanCars;
}
...
function carMaker(odometer) {
  // returns an object
  // the object has 2 properties:  odometer and drive
  // drive is a method, meaning it can be written as drive() outside of an object but while inside an object it is written as a function
  // drive is the property, the key of drive is a function
  // formula odometer = odometer + distance because the odometer increase with distance driven
  // this.odomenter means it is referring to the property inside of this object
